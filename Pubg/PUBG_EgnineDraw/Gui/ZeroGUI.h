...
namespace ZeroGUI
{
	namespace Colors
	{
		...
		FLinearColor Window_Background{ 0.009f, 0.009f, 0.009f, 0.5f };//
		FLinearColor Window_Header{ 0.10f, 0.15f, 0.84f, 0.5f }; //

		FLinearColor Button_Idle{ 0.10f, 0.15f, 0.84f, 0.5f };
		FLinearColor Button_Hovered{ 0.15f, 0.20f, 0.89f, 0.5f };
		FLinearColor Button_Active{ 0.20f, 0.25f, 0.94f, 0.5f };

		FLinearColor Checkbox_Idle{ 0.17f, 0.16f, 0.23f, 0.5f };
		FLinearColor Checkbox_Hovered{ 0.22f, 0.30f, 0.72f, 0.5f };
		FLinearColor Checkbox_Enabled{ 0.20f, 0.25f, 0.94f, 0.5f };
		...
		FLinearColor Slider_Idle{ 0.17f, 0.16f, 0.23f, 0.5f };
		FLinearColor Slider_Hovered{ 0.17f, 0.16f, 0.23f, 0.5f };
		FLinearColor Slider_Progress{ 0.22f, 0.30f, 0.72f, 0.5f };
		FLinearColor Slider_Button{ 0.10f, 0.15f, 0.84f, 0.5f };

		FLinearColor ColorPicker_Background{ 0.006f, 0.006f, 0.006f, 0.5f };
	}
	...
	FVector2D CursorPos()
	{
		POINT cursorPos = {};
		WINDOWINFO Info = {};
		Func::SGetWindowInfo(&Info);
		Func::SGetCursorPos(&cursorPos);
		return FVector2D{ float(cursorPos.x - Info.rcClient.left),float(cursorPos.y - Info.rcClient.top) };
	}
	...
	void Draw_Cursor(bool toogle)
	{
		if (toogle)
		{
			...
			UCanvas::DrawLine(cursorPos.X, cursorPos.Y, cursorPos.X + 35, cursorPos.Y + 10, 1, FLinearColor{ 0.30f, 0.30f, 0.80f, 1.0f });
			...
			while (y != 30) //20 steps
			{
				...
				UCanvas::DrawLine(cursorPos.X, cursorPos.Y, cursorPos.X + x, cursorPos.Y + y, 1, FLinearColor{ 0.30f, 0.30f, 0.80f, 1.0f });
			}

			UCanvas::DrawLine(cursorPos.X, cursorPos.Y, cursorPos.X + 15, cursorPos.Y + 30, 1, FLinearColor{ 0.30f, 0.30f, 0.80f, 1.0f });
			UCanvas::DrawLine(cursorPos.X + 35, cursorPos.Y + 10, cursorPos.X + 15, cursorPos.Y + 30, 1, FLinearColor{ 0.30f, 0.30f, 0.80f, 1.0f });
		}
	}
	...
	void TextLeftA(const char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		UCanvas::DrawText(pos.X, pos.Y, color, Colors::Text_Outline, name, false, true, true);
	}
	void TextLeft(const char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		UCanvas::DrawText(pos.X, pos.Y, color, Colors::Text_Outline, name, false, true, true);
	...
	void TextCenter(const char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		UCanvas::DrawText(pos.X, pos.Y, color, Colors::Text_Outline, name, true, true, true);
	}
	...
	void Draw_Line(FVector2D from, FVector2D to, int thickness, FLinearColor color)
	{
		UCanvas::DrawLine(from.X, from.Y, to.X, to.Y, thickness, color);
	}
	void drawFilledRect(FVector2D initial_pos, float w, float h, FLinearColor color)
	{
		UCanvas::DrawFilled(initial_pos.X, initial_pos.Y, w, h, color);
	...
	bool Window(const char* name, FVector2D* pos, FVector2D size, bool isOpen)
	{
		...
		//Drop last element
		if (current_element != -1 && !Func::GetAsyncKeyState2(0x1))
		{
			current_element = -1;
		}

		//Drag
		if (hover_element && Func::GetAsyncKeyState2(0x1))
		{
	...
	void Text(const char* text, int32_t Size)
	{
		elements_count++;
		...
		if (!sameLine)
			offset_y += size + padding.Y + 20;

		//Text
		FVector2D textPos = FVector2D{ pos.X + 5.0f, pos.Y + size / 2 + 30 };

		TextLeftA(text, textPos, FLinearColor{ 1.0f, 1.0f, 1.0f, 1.0f }, true);
		sameLine = false;
		last_element_pos = pos;
		//last_element_size = size;
		if (first_element_pos.X == 0.0f)
	...
	void SliderFloat(const char* name, float* value, float min, float max, const char* format = "%.0f"e)
	{
		elements_count++;

		FVector2D size = FVector2D{ 210, 20 };
		FVector2D slider_size = FVector2D{ 340, 7 };
		...
		//Text
		FVector2D textPos = FVector2D{ pos.X + slider_size.X + 5, /*pos.Y + 5 */pos.Y + slider_size.Y + 2.66f + padding.Y };
	...
	void Hotkey(const char* name, FVector2D size, int* key)
	{
		...
		if (active_hotkey == elements_count)
		{
			//Text
			FVector2D textPos = FVector2D{ pos.X + size.X / 2, pos.Y + size.Y / 2 };
			TextCenter("[Press Key]"e, textPos, FLinearColor{ 1.0f, 1.0f, 1.0f, 1.0f }, false);
			...
			if (!already_pressed)
			{
				...
				{
					if (Func::GetAsyncKeyState2(code))
					{
						*key = code;
						active_hotkey = -1;
					}
		...
		else
		{
			//Text
			FVector2D textPos = FVector2D{ pos.X + size.X / 2, pos.Y + size.Y / 2 };
			TextCenter(VirtualKeyCodeToString(*key).c_str(), textPos, FLinearColor{ 1.0f, 1.0f, 1.0f, 1.0f }, false);
